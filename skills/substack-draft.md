...
2. Research the topic:
   - Use Claude Code's built-in WebSearch for current sources (fall back to `tools/web-search.sh` if needed)
   - Use WebFetch to pull 2-3 key source pages (fall back to `tools/fetch-url.sh` if needed)
...
4. Save to articles/${today}-substack.md with YAML frontmatter:
   ...
   ---
   ```
5. Send a notification via `tools/notify.sh`:
   ```

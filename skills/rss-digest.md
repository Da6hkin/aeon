...
- Use WebFetch to pull the full article if the summary is too thin (fall back to `tools/fetch-url.sh` if needed)
- Write a 1-2 sentence summary of why it matters

Format and send via `tools/notify.sh` (under 4000 chars):
```
*RSS Digest — ${today}*

...
Steps:
1. Use WebSearch to find 5-8 current sources on the topic (fall back to `tools/web-search.sh`).
2. Search Semantic Scholar for relevant academic papers:
   ...
   ```
3. Use WebFetch (fall back to `tools/fetch-url.sh`) to read the 3-4 most relevant sources in depth.
...
6. Send an abbreviated summary via `tools/notify.sh`.

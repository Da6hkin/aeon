...
- Fetch the abstract (from API response or via WebFetch if needed; fall back to `tools/fetch-url.sh`)
...
Send abbreviated version via `tools/notify.sh` (under 4000 chars):
```
*Paper Digest — ${today}*
